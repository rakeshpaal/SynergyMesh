...
- ✅ 優先整合到既有目錄
- ✅ 遵循現有專案結構（core/, services/, automation/, governance/, config/,
  docs/, infrastructure/）
- ✅ 不隨意創建新頂層目錄
- ✅ 每個邏輯元件給出具體檔案路徑
...
- `docs/refactor_playbooks/03_refactor/meta/AI_PROMPTS.md`: 新增 HLP
  Executor 相關 AI 提示
...
- **原始**: `axiom-critical`
- **適配**: `system-cluster-critical` (K8s 標準) 或自定義
  `unmanned-island-critical`
...
1. **Quantum Backend Dependency**: 目前可能無 quantum-scheduler 實現
   ...
   - **配置**: `quantum_integration.enabled: false` +
