...
class PrivacyByDesign:
    ...
    def assess(self) -> Dict[str, float]:
        ...
        # 以用戶為中心分數
        self.user_centric_score = min(100, (consent_required / total_fields) * 200) if total_fields > 0 else 0
...
class PrivacyFramework:
    ...
    def check_compliance(
        ...
        if requirements.get('consent_required'):
            # 檢查是否有同意管理機制
            has_consent_mechanism = len(self.consent_manager.consents) > 0 or \
                                   any(f.requires_consent for f in self.privacy_design.data_fields.values())
    ...
    def perform_privacy_impact_assessment(self) -> Dict[str, Any]:
        ...
        high_sensitivity = sum(
            1 for f in self.privacy_design.data_fields.values()
            if f.sensitivity in [DataSensitivity.CONFIDENTIAL, DataSensitivity.RESTRICTED, DataSensitivity.TOP_SECRET]
        )
