# 回滚场景示例 (Rollback Scenario Example)

本文档提供了变更管理中回滚场景的详细示例，帮助团队理解如何计划和执行回滚。

## 场景 1: 应用版本回滚

### 背景

Payment Service 从 v1.2.0 升级到 v1.3.0 后，发现支付成功率从 99.8% 下降到 95%，需要立即回滚。

### 变更请求

```yaml
apiVersion: governance.machinenativeops.io/v1alpha1
kind: ChangeRequest
metadata:
  id: "CHG-2025-015"
  title: "Payment Service v1.3.0 升级"
  status: "rolled-back"
```

### 回滚触发条件

在实施计划中定义的触发条件：

```yaml
rollbackPlan:
  triggers:
    - "支付成功率 < 98% 持续 5 分钟"  # ✅ 触发
    - "平均响应时间 > 1000ms 持续 10 分钟"  # ❌ 未触发
    - "错误率 > 1% 持续 5 分钟"  # ❌ 未触发
```

**触发时间**: 2025-01-15 14:35:00 UTC

### 回滚执行

#### 第 1 步: 确认回滚决策 (2分钟)

```bash
# 14:35 - 确认指标异常
# Grafana 仪表板显示支付成功率下降到 95%

# 检查告警
FIRING: PaymentSuccessRateLow
  - Current: 95.2%
  - Threshold: 98%
  - Duration: 5m

# 决策: 立即回滚
# 决策人: SRE On-call + Team Lead
```

#### 第 2 步: 执行 Kubernetes 回滚 (3分钟)

```bash
# 14:37 - 执行回滚命令
kubectl rollout undo deployment/prod-payment-deploy -n production

# 输出
deployment.apps/prod-payment-deploy rolled back

# 监控回滚进度
kubectl rollout status deployment/prod-payment-deploy -n production

# 输出
Waiting for deployment "prod-payment-deploy" rollout to finish: 1 out of 3 new replicas have been updated...
Waiting for deployment "prod-payment-deploy" rollout to finish: 1 out of 3 new replicas have been updated...
Waiting for deployment "prod-payment-deploy" rollout to finish: 2 out of 3 new replicas have been updated...
Waiting for deployment "prod-payment-deploy" rollout to finish: 2 old replicas are pending termination...
Waiting for deployment "prod-payment-deploy" rollout to finish: 1 old replicas are pending termination...
deployment "prod-payment-deploy" successfully rolled out

# 14:40 - 回滚完成
```

#### 第 3 步: 验证回滚成功 (5分钟)

```bash
# 检查 Pod 状态
kubectl get pods -n production -l app=payment-api

# 输出
NAME                                      READY   STATUS    RESTARTS   AGE
prod-payment-deploy-v1.2.0-abc123-xyz1   1/1     Running   0          2m
prod-payment-deploy-v1.2.0-abc123-xyz2   1/1     Running   0          3m
prod-payment-deploy-v1.2.0-abc123-xyz3   1/1     Running   0          3m

# 检查版本
kubectl describe deployment prod-payment-deploy -n production | grep Image

# 输出
Image: payment-api:v1.2.0  # ✅ 已回滚到 v1.2.0

# 运行健康检查
curl https://api.example.com/health

# 输出
{
  "status": "healthy",
  "version": "v1.2.0"
}

# 14:45 - 验证通过
```

#### 第 4 步: 监控恢复 (10分钟)

```promql
# 监控支付成功率
(
  sum(rate(payment_success_total[5m])) /
  sum(rate(payment_total[5m]))
) * 100

# 14:50 时间点数据
Time: 14:50
Value: 99.7%  # ✅ 已恢复正常

# 15:00 时间点数据
Time: 15:00
Value: 99.8%  # ✅ 完全恢复
```

#### 第 5 步: 通知和记录 (持续)

```bash
# 14:40 - 发送 Slack 通知
# Channel: #incidents
Message:
🔄 [ROLLBACK] Payment Service 已回滚到 v1.2.0
原因: 支付成功率下降到 95%
影响: 14:35-14:40 (5分钟)
当前状态: 服务已恢复正常
后续: 将进行根因分析

# 14:45 - 更新变更请求
# 状态: rolled-back
# 添加回滚记录

# 15:00 - 发送恢复确认
# Channel: #incidents
Message:
✅ [RESOLVED] Payment Service 已完全恢复
支付成功率: 99.8%
影响时长: 5 分钟
受影响交易: ~50 笔 (已重试成功)
```

### 总时长

- **检测到问题**: 14:30 (自动告警)
- **决策回滚**: 14:35 (5分钟)
- **执行回滚**: 14:37-14:40 (3分钟)
- **验证通过**: 14:45 (5分钟)
- **完全恢复**: 15:00 (10分钟)

**总计**: 从检测到完全恢复 30 分钟

### 根因分析

```yaml
事后分析会议:
  时间: 2025-01-16 10:00
  参与: Payment Team, SRE, QA

根本原因:
  - v1.3.0 引入的新重试逻辑有 bug
  - 在某些失败场景下导致重复扣款
  - Payment Gateway 检测到异常并拒绝部分请求

为什么没有在测试中发现:
  - 测试环境的 Payment Gateway 配置与生产不同
  - 测试数据没有覆盖这个特定的失败场景
  - 压力测试规模不够

改进措施:
  1. 增加生产环境的测试用例
  2. 使用生产流量的 shadow testing
  3. 增强压力测试场景
  4. Payment Gateway 配置同步到测试环境
  5. 增加重试逻辑的单元测试覆盖

预防措施:
  - 实施金丝雀发布（先 10% 流量）
  - 增加自动回滚触发器
  - 改进告警灵敏度
```

### 经验教训

#### ✅ 做得好的地方

1. **快速决策**: 5分钟内决定回滚
2. **清晰的回滚计划**: 提前定义好触发条件和步骤
3. **自动化工具**: Kubernetes 一键回滚
4. **有效沟通**: 及时通知所有相关方
5. **完整记录**: 详细的时间线和决策记录

#### ❌ 需要改进的地方

1. **测试覆盖**: 生产环境配置未同步到测试
2. **发布策略**: 应该使用金丝雀发布
3. **告警延迟**: 5分钟才触发告警，可以更快
4. **影响评估**: 没有提前评估回滚的业务影响

---

## 场景 2: 数据库 Schema 变更回滚

### 背景

User Service 数据库 schema 变更后，发现新字段导致查询性能严重下降，需要回滚 schema。

### 挑战

**难点**: 数据库 schema 回滚比应用回滚复杂得多：
- 可能已有新数据写入新字段
- 向后兼容性问题
- 需要数据迁移

### 回滚策略

#### 策略 1: 前向修复 (优先)

```sql
-- 不回滚 schema，而是优化查询
-- 添加索引解决性能问题
CREATE INDEX idx_users_new_field ON users(new_field);

-- 分析查询计划
EXPLAIN ANALYZE SELECT * FROM users WHERE new_field = 'value';
```

**决策**: 如果能在 1 小时内修复，优先前向修复

#### 策略 2: Schema 回滚 (最后手段)

```yaml
回滚步骤:
  1. 应用回滚到兼容旧 schema 的版本
  2. 确认没有代码依赖新字段
  3. 导出新字段数据（如果需要保留）
  4. 执行 schema 回滚
  5. 验证应用功能
```

```sql
-- 步骤 3: 备份新字段数据
CREATE TABLE users_new_field_backup AS
SELECT id, new_field, created_at
FROM users
WHERE new_field IS NOT NULL;

-- 步骤 4: 删除新字段
ALTER TABLE users DROP COLUMN new_field;

-- 验证
\d users
```

### 实际执行

本次选择了**前向修复**：

```sql
-- 添加复合索引
CREATE INDEX CONCURRENTLY idx_users_email_new_field
ON users(email, new_field);

-- 查询性能对比
-- 修复前: 2500ms
-- 修复后: 45ms

-- ✅ 问题解决，无需回滚
```

---

## 场景 3: 配置变更回滚

### 背景

更新 Nginx ingress 配置后，导致所有请求返回 502 错误。

### 回滚执行

```bash
# 立即回滚配置
kubectl rollout undo deployment/ingress-nginx-controller -n ingress-nginx

# 或者：应用之前的配置文件
kubectl apply -f backup/ingress-config-previous.yaml

# 验证
curl -I https://api.example.com

# 输出
HTTP/2 200 OK  # ✅ 已恢复
```

### 时长

- **检测**: 30 秒（所有请求立即失败）
- **回滚**: 1 分钟
- **恢复**: 2 分钟

**总计**: 3 分钟内完全恢复

---

## 回滚最佳实践

### 1. 计划阶段

```yaml
回滚计划必须包含:
  - [ ] 明确的触发条件
  - [ ] 详细的回滚步骤
  - [ ] 预估的回滚时间
  - [ ] 回滚后的验证方法
  - [ ] 回滚的潜在影响
  - [ ] 数据一致性处理
  - [ ] 通知计划
```

### 2. 执行阶段

```yaml
回滚执行原则:
  - 快速决策: 满足触发条件立即执行
  - 清晰沟通: 通知所有相关方
  - 完整记录: 记录所有操作和时间
  - 持续监控: 确保回滚成功
  - 保留证据: 便于事后分析
```

### 3. 验证阶段

```yaml
回滚验证清单:
  - [ ] 应用版本正确
  - [ ] 所有 Pod 健康
  - [ ] 健康检查通过
  - [ ] 关键指标恢复正常
  - [ ] 功能测试通过
  - [ ] 无新的告警
  - [ ] 用户流量正常
```

### 4. 事后分析

```yaml
事后分析重点:
  - 根本原因是什么？
  - 为什么没在测试中发现？
  - 回滚执行是否顺利？
  - 哪些可以改进？
  - 如何预防类似问题？
```

---

## 回滚决策树

```
问题检测
  ↓
严重程度评估
  ├─ 严重 (P0/P1)
  │   ├─ 有快速修复？
  │   │   ├─ 是 → 前向修复 (< 5分钟)
  │   │   └─ 否 → 立即回滚
  │   └─ 回滚
  │
  └─ 中等/低 (P2/P3)
      ├─ 有修复方案？
      │   ├─ 是 → 评估修复时间
      │   │   ├─ < 1小时 → 前向修复
      │   │   └─ > 1小时 → 回滚
      │   └─ 否 → 回滚
      └─ 回滚

回滚决策原则:
  - P0/P1: 优先快速恢复服务
  - P2/P3: 权衡修复成本和影响
  - 不确定时: 选择回滚
```

---

## 参考资料

- [变更管理文档](../../docs/governance/05-change-management.md)
- [变更请求模板](../../templates/governance/forms/change-request.template.yaml)
- [变更请求示例](./CHG-2025-001.yaml)
